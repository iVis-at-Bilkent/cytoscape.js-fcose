...
var coseLayout = function coseLayout(options, spectralResult) {
  ...
  var _processChildrenList = function processChildrenList(parent, children, layout, options) {
    ...
    for (var i = 0; i < size; i++) {
      ...
      if (options.fixedPositionOnBoundary) {
        var fixedPos = optFn(options.fixedPositionOnBoundary, theChild);
        if (fixedPos) {
  ...
  if (options.boundaryNodeConstraint) {
    nodes.forEach(function (theChild) {
      ...
      if (boundedNodeCy) {
        var boundedNodeCose = idToLNode[boundedNodeCy.id()];
        var theNode = idToLNode[theChild.data("id")];
        if (boundedNodeCose && theNode) {
          var parentGraph = boundedNodeCose.child;
          if (!parentGraph) {
            parentGraph = coseLayout.getGraphManager().add(coseLayout.newGraph(), boundedNodeCose);
          }
          theNode.boundaryGraph = parentGraph;
          parentGraph.boundaryNodes.push(theNode);
        }
